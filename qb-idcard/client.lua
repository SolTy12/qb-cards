...
RegisterNetEvent('sh-idcard:client:showCard', function(cardData, mugShotSupport)
    ...
    if cfg.closeInventoryOnUse then
        -- Close QB Inventory
        if QBCore then
            TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[cardData.item or 'id_card'], "use")
            ExecuteCommand('closeInventory')
        end
        ...
        -- Close qb-inventory
        TriggerEvent('qb-inventory:client:close')
        
        -- Close qs-inventory
        TriggerEvent('qs-inventory:client:close')
        
        -- Close ps-inventory
        TriggerEvent('ps-inventory:client:close')
